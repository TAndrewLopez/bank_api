...
const handleError = (response: Response, message: string, statusCode: number = 400) => {
    return response.status(statusCode).json({ status: false, message });
};

const handleSuccess = (response: Response, message: string, data = {}, statusCode: number = 200) => {
